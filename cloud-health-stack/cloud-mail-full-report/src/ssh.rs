...
echo "a001 CAPABILITY" | timeout $T openssl s_client -connect localhost:993 -quiet 2>/dev/null | head -3
echo "===postfixQueue==="
curl -skf -u "admin:$ADMIN_CREDS" https://localhost:443/api/queue/messages 2>/dev/null | head -3 || echo "empty"
echo "===rspamd==="
...
echo "===users==="
curl -skf -u "admin:$ADMIN_CREDS" https://localhost:443/api/principal 2>/dev/null | head -5 || echo "0"
echo "===smtp25==="
...
curl -skf -u "admin:$ADMIN_CREDS" https://localhost:443/api/principal 2>/dev/null | head -5 || echo "API_FAIL"
echo "===stalwartApiDomains==="
curl -skf -u "admin:$ADMIN_CREDS" https://localhost:443/api/domain 2>/dev/null | head -5 || echo "API_FAIL"
